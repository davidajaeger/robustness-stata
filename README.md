...
The Wald statistic, its bootstrap p-value, and the Wald-based bound (`W*`) are also computed and returned in `r()`. The Wald requires a full-rank contrast covariance. When the specifications are collinear in the draws (for example one is a fixed shift or a linear combination of others) that covariance is singular, and `W`, `p_W`, and `W*` are returned as missing with a warning rather than computed with a generalized inverse. The range statistics (`R`, `p_R`, `R*`) do not use the contrast covariance and are unaffected. Exact duplicate columns are a separate case, rejected when the draws are read.
